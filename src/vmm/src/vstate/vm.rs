...
impl Vm {
    ...
    /// Takes a snapshot of guest memory. For Diff/Full, writes to file.
    /// For Msync/MsyncAndState, uses msync to sync shared memory to backing file.
        ...
        match snapshot_type {
            SnapshotType::Diff | SnapshotType::Full => {
                ...
                if file_existed {
                    ...
                    // - For full snapshots, the entire file's contents will be overwritten anyway.
                    //   We have to avoid truncating here to deal with the edge case where it
                    //   represents the snapshot file from which this very microVM was loaded (as
                    //   modifying the memory file would be reflected in the mmap of the file,
                    //   meaning a truncate operation would zero out guest memory, and thus corrupt
                    //   the VM).
            ...
            SnapshotType::Msync | SnapshotType::MsyncAndState => {
                // For msync-based snapshots, we synchronize memory to the backing file
                // using msync(). This is used for live migration when memory is backed
                // by a shared file (MAP_SHARED).
