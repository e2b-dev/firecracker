...
def test_memory_post_boot(uvm_plain):
    ...
    for i in range(len(empty_bitmap)):
        assert (empty_bitmap[i] & resident_bitmap[i]) == empty_bitmap[
            i
...
def test_memory_benchmark(microvm_factory, guest_kernel_linux_6_1, rootfs):
    ...
    try:
        ...
        mem_size_mib = 256
    test_microvm.basic_config(
        mem_size_mib=mem_size_mib, huge_pages=HugePagesConfig.HUGETLBFS_2MB
    )
    ...
    test_microvm.ssh.run(
        "dd if=/dev/zero of=/dev/shm/zero_mem bs=1M count={} 2>/dev/null || true".format(
            fault_memory_mib
        )
    )
    ...
    print(f"{'='*60}")
    print(
        f"Total Memory: {total_memory_mib:.2f} MiB ({total_memory_bytes / (1024**3):.3f} GB)"
    )
    print(
        f"Resident Pages: {resident_page_count} / {total_pages} ({resident_page_count * 100 / total_pages:.1f}%)"
    )
    print(
        f"Resident Memory: {resident_memory_mib:.2f} MiB ({resident_memory_bytes / (1024**3):.3f} GB)"
    )
    print(
        f"Empty Pages: {empty_page_count} / {resident_page_count} ({empty_page_count * 100 / resident_page_count if resident_page_count > 0 else 0:.1f}% of resident)"
    )
